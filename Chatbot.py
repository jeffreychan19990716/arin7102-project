...
class ChatbotApp(QtWidgets.QWidget):
    def __init__(self, online):
        ...
        self.esg_database = pd.read_csv("data/SP 500 ESG Risk Ratings.csv")
    ...
    def add_message(self, sender, message):
        ...
        if sender == "Chatbot":
            profile_pixmap = QPixmap("src/Chatbot icon transparent.png")
            ...
            profile_pixmap = QPixmap("src/profile icon.png")
            profile_icon.setPixmap(profile_pixmap.scaled(64, 64, Qt.KeepAspectRatio, Qt.SmoothTransformation))
            layout.addWidget(label)
            layout.addWidget(profile_icon)
    ...
    def check_for_url(self, text): 
        # using regular expression to filter url
    ...
    def generate_esg_report(self, ticker, success, summary, outputs):
        # generate esg report
        if not success:
            outputs.append("Failed to analyse as news not related to ESG. Try again!")
            return outputs
        ...
        if ticker is not None:
            try:
                ...
                new_esg = self.model.calculate_overall_esg(stable_esg, esg)
    ...
    def message_handling(self, user_message):
        ...
        if self.online:
            ...
            data = feeder.get()
            success, summary, response = feeder.analyse_news(data, index=index)
        else:
...
if __name__ == "__main__":
    ...
    font = QFont("Calibri")
    app.setFont(font)
    window = ChatbotApp(online=args.online)
    window.setWindowIcon(QIcon('src/Chatbot icon.png'))
    window.reset()
    window.show()
    app.exec_()
    sys.exit()
